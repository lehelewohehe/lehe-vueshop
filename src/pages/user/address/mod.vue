<template>
    <div class="page">
        <my-header :isLeft="true" :title="'修改收货地址'" :isRight="true" :right="'删除'" @click="deleteAddressOne" class="ucenter-header"></my-header>
        <div class="add-main">
            <ul>
                <li>收货人</li>
        ...
        <div class="save-btn-wrapper">
            <my-button class="save-btn" @click="saveModAddress">修改</my-button>
        </div>
...
export default {
    ...
    created() {
        ...
        this.getAddressOne({data: {uid: this.loginInfo.uid, aid: this.$route.query.aid}, success: (data) => {
        	...
        	this.areaValue = this.province + this.city + this.area
        }})
    },
    methods: {
        ...mapActions({
            addAddressOne: 'address/addAddressOne',
            getAddressOne: 'address/getAddressOne',
            modAddressOne: 'address/modAddressOne',
            delAddressOne: 'address/delAddressOne'
        }),
        toggleAreaList(flag) {
            this.isArea = flag
        },
        ...
        deleteAddressOne() {
        	this.delAddressOne({data: {uid: this.loginInfo.uid, aid: this.$route.query.aid}, success: (data) => {
            	this.$router.go(-1)
            	this.$toast(data)
            }})
        },
        saveModAddress() {
            ...
            this.modAddressOne({ data: {aid: this.$route.query.aid, uid: this.loginInfo.uid, name: this.name, cellphone: this.cellPhone, province: this.province , city: this.city, area: this.area, address: this.detailAddress, isdefault: this.isDefault ? '1' : '0'}, success: (data) => {
            	this.$router.go(-1)
            	this.$toast(data)
            } })
        }
